...
class MinEntLoss(torch.nn.Module):
    """
...
def mpcl_loss_calc(feas, labels, class_center_feas, loss_func, pixel_sel_loc=None, tag='source'):
    ...
    loss =  loss_func(feas,labels,class_center_feas,pixel_sel_loc=pixel_sel_loc)
    return loss

class MPCL(torch.nn.Module):
