...
class ResNetMulti(nn.Module):
    ...
    def forward(self, x, require_embedding_feature=False):
        _, _, h, w = x.size()
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.layer3(x)
        embedding_feature = self.layer4(x)
        x1 = self.layer5(x)

        x2 = torch.cat((embedding_feature,x), 1)
        x2 = self.layer6(x2)
        # aux
        x1 = nn.functional.interpolate(x1, size=(h, w), mode='bilinear', align_corners=True)
        # main
        x2 = nn.functional.interpolate(x2, size=(h, w), mode='bilinear', align_corners=True)
        if require_embedding_feature:
            return embedding_feature, x1, x2
        else:
            return x1, x2
    ...
    def optim_parameters(self, args):
        return [{'params': self.get_1x_lr_params_NOscale(), 'lr': args.learning_rate},
                {'params': self.get_10x_lr_params(), 'lr': args.learning_rate * 10}]
...
def DeeplabMulti(num_classes=21, use_se = False, train_bn = False, norm_style = 'bn', droprate = 0.1, init_weights = None, restore_from = None):
    ...
    if restore_from is None:
        if init_weights is not None:
            saved_state_dict = torch.load(init_weights)
            print('load pretrained model from:', init_weights)
...
if __name__ == '__main__':
    model = ResNetMulti(Bottleneck, [3, 4, 23, 3], 1)
